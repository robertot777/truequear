...
public class ComunaResourceIntTest {
     ...
     *
     * @see truequear.repository.search.ComunaSearchRepositoryMockConfiguration
     */
    @Autowired
    ...
    public void createComuna() throws Exception {
        ...
        // Validate the Comuna in Elasticsearch
        verify(mockComunaSearchRepository, times(1)).save(testComuna);
    }
    ...
    public void createComunaWithExistingId() throws Exception {
        ...
        // Validate the Comuna in Elasticsearch
        verify(mockComunaSearchRepository, times(0)).save(comuna);
    }
    ...
    public void deleteComuna() throws Exception {
        ...
        // Validate the Comuna in Elasticsearch
        verify(mockComunaSearchRepository, times(1)).deleteById(comuna.getId());
    }

    @Test
    @Transactional
    public void searchComuna() throws Exception {
        // Initialize the database
        comunaRepository.saveAndFlush(comuna);
        when(mockComunaSearchRepository.search(queryStringQuery("id:" + comuna.getId())))
            .thenReturn(Collections.singletonList(comuna));
        // Search the comuna
        restComunaMockMvc.perform(get("/api/_search/comunas?query=id:" + comuna.getId()))
            .andExpect(status().isOk())
            .andExpect(content().contentType(MediaType.APPLICATION_JSON_UTF8_VALUE))
            .andExpect(jsonPath("$.[*].id").value(hasItem(comuna.getId().intValue())))
            .andExpect(jsonPath("$.[*].nombreComuna").value(hasItem(DEFAULT_NOMBRE_COMUNA)));
    }
