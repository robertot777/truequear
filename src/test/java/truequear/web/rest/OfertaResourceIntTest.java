...
public class OfertaResourceIntTest {
    ...
    private static final byte[] UPDATED_GENERA_OFERTA = TestUtil.createByteArray(1, "1");
     ...
     *
     * @see truequear.repository.search.OfertaSearchRepositoryMockConfiguration
     */
    @Autowired
    ...
    public void searchOferta() throws Exception {
        // Initialize the database
        ofertaRepository.saveAndFlush(oferta);
        when(mockOfertaSearchRepository.search(queryStringQuery("id:" + oferta.getId())))
            .thenReturn(Collections.singletonList(oferta));
        // Search the oferta
        restOfertaMockMvc.perform(get("/api/_search/ofertas?query=id:" + oferta.getId()))
            .andExpect(status().isOk())
            .andExpect(content().contentType(MediaType.APPLICATION_JSON_UTF8_VALUE))
            .andExpect(jsonPath("$.[*].id").value(hasItem(oferta.getId().intValue())))
            .andExpect(jsonPath("$.[*].generarOferta").value(hasItem(DEFAULT_GENERAR_OFERTA)))
            .andExpect(jsonPath("$.[*].tipoObjeto").value(hasItem(DEFAULT_TIPO_OBJETO.toString())))
            .andExpect(jsonPath("$.[*].generaOfertaContentType").value(hasItem(DEFAULT_GENERA_OFERTA_CONTENT_TYPE)))
            .andExpect(jsonPath("$.[*].generaOferta").value(hasItem(Base64Utils.encodeToString(DEFAULT_GENERA_OFERTA))));
    }
