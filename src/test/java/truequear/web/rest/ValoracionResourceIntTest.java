...
public class ValoracionResourceIntTest {
     ...
     *
     * @see truequear.repository.search.ValoracionSearchRepositoryMockConfiguration
     */
    @Autowired
    ...
    public void createValoracion() throws Exception {
        ...
        // Validate the Valoracion in Elasticsearch
        verify(mockValoracionSearchRepository, times(1)).save(testValoracion);
    }
    ...
    public void createValoracionWithExistingId() throws Exception {
        ...
        // Validate the Valoracion in Elasticsearch
        verify(mockValoracionSearchRepository, times(0)).save(valoracion);
    }
    ...
    public void deleteValoracion() throws Exception {
        ...
        // Validate the Valoracion in Elasticsearch
        verify(mockValoracionSearchRepository, times(1)).deleteById(valoracion.getId());
    }

    @Test
    @Transactional
    public void searchValoracion() throws Exception {
        // Initialize the database
        valoracionRepository.saveAndFlush(valoracion);
        when(mockValoracionSearchRepository.search(queryStringQuery("id:" + valoracion.getId())))
            .thenReturn(Collections.singletonList(valoracion));
        // Search the valoracion
        restValoracionMockMvc.perform(get("/api/_search/valoracions?query=id:" + valoracion.getId()))
            .andExpect(status().isOk())
            .andExpect(content().contentType(MediaType.APPLICATION_JSON_UTF8_VALUE))
            .andExpect(jsonPath("$.[*].id").value(hasItem(valoracion.getId().intValue())))
            .andExpect(jsonPath("$.[*].tipoValoracion").value(hasItem(DEFAULT_TIPO_VALORACION.toString())));
